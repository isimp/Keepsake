...
1. Set the new version in `Directory.Build.props` (`<Version>`), `src/Keepsake/Plugin.cs` (`public const string Version`) and `packaging/manifest.json` (`version_number`). The workflow checks that all three agree.
...
| GitHub release | Tags `vX.Y.Z` and attaches the Hexium zip. |
...
Keepsake is client-side only. Servers do not need it.
...
The script reads the installed Valheim, BepInEx and Jotunn, replaces every method body with `throw null` and writes the result to `lib/`. The stubs can be compiled against but not run. If you add a reference to either project, add it to the list in the script as well.
